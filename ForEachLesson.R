#Matt Kustra 4/20/21
#tutorial on how to use foreach to parallel compute
rm(list=ls())
...
forresult<-rep(NULL,length(numbers))
system.time(
...
#the strucutre is foreach(iterator=xxx) %operator% {expressiion}
#different operators, mainly %do% and %dopar%
#%do% is not parallel and similar to apply family but looks more like a typical forloop 
#%dopar% is parallel
...
#I ususally use less than max, so I can do other things on my computer.
...
#by defualt it's a lits of results, which isn't that great
...
#althoughh simple, like a for loop inside the expression {} we can do any number of tasks
...
Resultsd
# 7. Nesting foreach ------------------------------------------------------
#we can nest foreachh with the %:% operator
